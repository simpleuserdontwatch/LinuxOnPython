...
def savehhd():
    with open('LinuxOnPythonSave.hhd', 'a+b') as f:
        pickle.dump(filesystem,f)
def savehhdandexit():
    with open('LinuxOnPythonSave.hhd', 'a+b') as f:
        f.truncate(0)
...
root.protocol("WM_DELETE_WINDOW", savehhdandexit)
def do_popup(event):
    try:
...
filesystem = {"home":
              {"user": {}},
              "etc": {"hostname": "localhost", ".prefix": "$ ", "ver": "11.0 Butterfly"},
...
output+='Hello, World!'""",},
              "boot": {"kernel": """
# Note: You can do scripting here and make a custom bootup script
print('Loaded kernel')
""",
                       "INIT.ly": """
mount /dev
...
try:
    ...
    try:
        with open('LinuxOnPythonSave.hhd', 'rb') as f:
                filesystem = pickle.load(f)
    except:
        try:
            with open('LinuxOnPython11.0.hhd', 'rb') as f:
                filesystem = pickle.load(f)
        except:
            print('Unable to load HHD. Falling back to original filesystem')
pwd = '/home/user/'
...
def processkey(event):
    ...
    if event.keysym == 'BackSpace':
        ...
        if not (inp == '' or inp[0:1] == ' '):
            output+='\n'+pwd.replace(homedir, '~/')+filesystem['etc']['.prefix']+inp+'\n'
        else:
            output+='\n'+pwd.replace(homedir, '~/')+filesystem['etc']['.prefix']
            return
        emulate(inp)
        inp = ''
def emulate(command):
    global output, win, curdir, lastdir, filesystem, pwd, temp, temp2
    try:
        if logger:
            print()
        cmdandargs = command.split()
        ...
        if cmd == 'cd':
            ...
            try:
                if args[-1][0] != '/' and args[-1][0] != '$':
                    curdir[args[-1]] = curdir[args[-1]]+'\n'+' '.join(args[:-1])
                elif args[-1][0] == '$':
                    temp = filesystem
                    for i in env[args[-1][1:]].split('/')[1:-1]:
                        temp = temp[i]
                        print(temp, i)
                    temp[env[args[-1][1:]].split('/')[-1]] = temp[env[args[-1][1:]].split('/')[-1]]+'\n'+' '.join(args[:-1])
                else:
                    temp = filesystem
                    for i in args[-1].split('/')[1:-1]:
                        temp = temp[i]
                        print(temp, i)
                    print(temp)
                    temp[args[-1].split('/')[-1]] = temp[args[-1].split('/')[-1]].split('/')[-1]+'\n'+' '.join(args[:-1])
                    print(temp)
            except: pass
        elif cmd == 'rem':
            try:
                if args[-1][0] != '/' and args[-1][0] != '$':
                    ...
                    for i in env[args[-1][1:]].split('/')[1:-1]:
                        temp = temp[i]
                        print(temp, i)
                    temp[env[args[-1][1:]].split('/')[-1]] = temp[env[args[-1][1:]].split('/')[-1]][:temp[env[args[-1][1:]].split('/')[-1]].rfind('\n')]
                else:
                    temp = filesystem
                    for i in args[-1].split('/')[1:-1]:
                        temp = temp[i]
                        print(temp, i)
                    print(temp)
                    temp[args[-1].split('/')[-1]] = temp[args[-1].split('/')[-1]].split('/')[-1][:temp[args[-1].split('/')[-1]].split('/')[-1]].rfind('\n')
                    print(temp)
            except: pass
        elif cmd == 'touch':
            try:
            ...
            try:
                if args[0] in curdir:
                    for i in curdir[args[0]].splitlines():
                        emulate(i)
                else:
                    output += 'E: File doesnt exist!'
            except:
            ...
            output += f'Operating system: LinuxOnPython {filesystem["etc"]["ver"]}\nShell: MSDOS mode shell'
        elif cmd == 'res':
...
mkdir <name> - Makes a empty directory
rename <file> <new file> - Renames a file
exec <file> - Execute a .sh script file
clear - Clears screen
...
add <contents> <file> - Adds a line to file, useful for multiple line files
rem <file> - Removes last line from a file"""
...
#-Welcome screen-
welcome = 'Welcome to LinuxOnPython 11.0 Butterfly!\nToday date is '+datetime.today().strftime('%Y-%m-%d')
#----------------
...
def update():
    ...
    if curstate:
        C.create_text(padx,pady,fill=fontcol,font=(fontnam, fontsiz, fontsty),
            text=output+'\n'+pwd.replace(homedir, '~/')+filesystem['etc']['.prefix']+inp+'_', anchor="nw", tags="out")
    else:
        C.create_text(padx,pady,fill=fontcol,font=(fontnam, fontsiz, fontsty),
            text=output+'\n'+pwd.replace(homedir, '~/')+filesystem['etc']['.prefix']+inp, anchor="nw", tags="out")
...
def fakeboot():
    C.delete('all')
    C.create_text(padx,pady,fill=fontcol,font=(fontnam, fontsiz, fontsty), text="Booting the Pykernel..", anchor="nw")
    root.after(50,fakeboot1)
def fakeboot1():
    C.delete('all')
    try:
        exec(filesystem['boot']['kernel'])
        C.create_text(padx,pady,fill=fontcol,font=(fontnam, fontsiz, fontsty), text="Booting the Pykernel..\t\t[Done]", anchor="nw")
        root.after(300,fakeboot2)
    except:
        C.create_text(padx,pady,fill=fontcol,font=(fontnam, fontsiz, fontsty), text="Booting the Pykernel..\t\t[Failed]\nPykernel was corrupted/deleted. Please contact your administrator for help\nSystem halted", anchor="nw")
def fakeboot2():
    C.delete('all')
    C.create_text(padx,pady,fill=fontcol,font=(fontnam, fontsiz, fontsty), text="Booting the Pykernel..\t\t[Done]\nStarting the init process", anchor="nw")
    root.after(100,fakeboot3)
def fakeboot3():
    C.delete('all')
    C.create_text(padx,pady,fill=fontcol,font=(fontnam, fontsiz, fontsty), text="Booting the Pykernel..\t\t[Done]\nStarting the init process\nMounting volumes...", anchor="nw")
    root.after(150,fakeboot4)
def fakeboot4():
    C.delete('all')
    C.create_text(padx,pady,fill=fontcol,font=(fontnam, fontsiz, fontsty), text="Booting the Pykernel..\t\t[Done]\nStarting the init process\nMounting volumes...\t\t[Done]", anchor="nw")
    root.after(50,fakeboot5)
def fakeboot5():
    C.delete('all')
    C.create_text(padx,pady,fill=fontcol,font=(fontnam, fontsiz, fontsty), text="Booting the Pykernel..\t\t[Done]\nStarting the init process\nMounting volumes...\t\t[Done]\nStarting shell...", anchor="nw")
